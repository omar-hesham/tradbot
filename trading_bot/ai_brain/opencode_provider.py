...
class OpenCodeProvider(BaseAIProvider):
    def __init__(
        ...
        self.api_key = (
            api_key
            or get_credential("OPENCODE_API_KEY")
            or os.environ.get("OPENCODE_API_KEY")
        )
        self.model = (
            model
            or get_credential("OPENCODE_MODEL")
            or os.environ.get("OPENCODE_MODEL")
            or DEFAULT_MODEL
        )
        self.base_url = (
            base_url
            or get_credential("OPENCODE_BASE_URL")
    ...
    async def ask(self, system_prompt: str, user_prompt: str) -> str:
        ...
        try:
            ...
            import json
            return json.dumps({"action": "HOLD", "error": f"OpenCode API error: {str(e)}"})

    async def test_connection(self) -> dict:
        if not self.api_key:
            return {"connected": False, "error": "OpenCode API key not configured"}
    ...
    async def list_models(self) -> list[dict]:
        ...
        try:
            ...
            return [{"name": model_id} for model_id in model_ids]
        except Exception as e:
...
def is_configured() -> bool:
    return (
        get_credential("OPENCODE_API_KEY") is not None
        or os.environ.get("OPENCODE_API_KEY") is not None
    )
